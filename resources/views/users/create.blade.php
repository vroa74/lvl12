    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <flux:subheading>{{ __('Completa el formulario para crear un nuevo usuario') }}</flux:subheading>
            </div>
        ...
        <div class="max-w-3xl">
            <form action="{{ route('users.store') }}" method="POST" class="space-y-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
                @csrf
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                        <flux:select label="Cargo" name="cargo" required>
                            ...
                            <option value="subdirector" {{ old('cargo') == 'subdirector' ? 'selected' : '' }}>Subdirector</option>
                            <option value="trabajo social" {{ old('cargo') == 'trabajo social' ? 'selected' : '' }}>Trabajo Social</option>
                            <option value="prefecto" {{ old('cargo') == 'prefecto' ? 'selected' : '' }}>Prefecto</option>
                            <option value="administrativo" {{ old('cargo') == 'administrativo' ? 'selected' : '' }}>Administrativo</option>
                            <option value="padre de familia" {{ old('cargo') == 'padre de familia' ? 'selected' : '' }}>Padre de Familia</option>
                        </flux:select>
                        @error('cargo')
                            <flux:text class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</flux:text>
                        @enderror
                    ...
                    <div>
                        <flux:select label="Tema" name="theme" required>
                            <option value="dark" {{ old('theme', 'dark') == 'dark' ? 'selected' : '' }}>Oscuro</option>
